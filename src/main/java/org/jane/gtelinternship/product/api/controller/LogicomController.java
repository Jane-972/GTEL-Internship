...
import org.jane.gtelinternship.product.api.dto.response.SimpleProductResponseDto;
...
public class LogicomController {
  ...
  @GetMapping("/products")
  public Stream<SimpleProductResponseDto> getFirst10Products() {
    return logicomService.getFirst10Products().map(SimpleProductResponseDto::fromModel);
  }

  @GetMapping("/products/full")
  public Stream<ProductResponseDto> getFirst10ProductsFull() {
    return logicomService.getFirst10ProductsFull().map(ProductResponseDto::fromModel);
  }

  @GetMapping("/products/full/{sku}")
  public ProductResponseDto getProductFullBySku(@PathVariable String sku) {
    FullProduct fullProduct = logicomService.getProductFullBySku(sku);
